...
function main()
    ...
    for row in eachrow(case_studies_info)
        ...
        for rp in representative_periods
            ...
            # ------------------------------------------------------------------
            # Cache clustering output from the FULL scenario set.
            # The reduced scenario model will reuse these exact representative
            # periods and mappings, instead of clustering the reduced set again.
            # Scenario IDs are kept unchanged throughout.
            # ------------------------------------------------------------------

            clustering_cache_dir = joinpath(
                homedir(),
                "Nextcloud",
                "ExperimentData",
                "NL-output-data",
            ...
            mkpath(clustering_cache_dir)

            clustered_profiles_df =
                TIO.get_table(connection_benchmark, "profiles_rep_periods")
            rep_periods_mapping_df =
                TIO.get_table(connection_benchmark, "rep_periods_mapping")
            rep_periods_data_df =
                TIO.get_table(connection_benchmark, "rep_periods_data")
            timeframe_data_df =
                TIO.get_table(connection_benchmark, "timeframe_data")

            CSV.write(
                joinpath(clustering_cache_dir, "profiles_rep_periods.csv"),
                clustered_profiles_df;
                writeheader=true,
            )

            CSV.write(
                joinpath(clustering_cache_dir, "rep_periods_mapping.csv"),
                rep_periods_mapping_df;
                writeheader=true,
            )

            CSV.write(
                joinpath(clustering_cache_dir, "rep_periods_data.csv"),
                rep_periods_data_df;
                writeheader=true,
            )

            CSV.write(
                joinpath(clustering_cache_dir, "timeframe_data.csv"),
                timeframe_data_df;
                writeheader=true,
            )

            # Restore the original long profiles after ratio-based clustering.
            if use_ratio == true
                DuckDB.query(
                    connection_benchmark,
            ...
            # Cache the restored long profiles. These are needed later, e.g. for
            # fixing seasonal-storage levels between reduced and full models.
            clustered_profiles_full_df =
                TIO.get_table(connection_benchmark, "profiles")

            CSV.write(
                joinpath(clustering_cache_dir, "profiles.csv"),
                clustered_profiles_full_df;
                writeheader=true,
            )
            ...
            for solver in solvers
                ...
                # Keep the ORIGINAL scenario IDs. This is essential because the
                # reduced model must reuse the representative periods belonging
                # to those exact scenarios in the full-set clustering.
                ...
                # ------------------------------------------------------------------
                # Reuse clustering from the FULL scenario set.
                #
                # No second TC.cluster! call is needed. Keep the original scenario
                # IDs and retain only the rows belonging to selected scenarios.
                # Thus scenario s gets exactly the representative periods and
                # period mapping that were obtained for scenario s in the full set.
                # ------------------------------------------------------------------

                time_to_cluster = 0.0

                profiles_rep_periods_red = filter(
                    row -> row.scenario in selected_scenarios_ids,
                    clustered_profiles_df,
                )

                rep_periods_mapping_red = filter(
                    row -> row.scenario in selected_scenarios_ids,
                    rep_periods_mapping_df,
                )
                ...
                # These two tables describe the temporal representative-period
                # structure generated by the full-set clustering. They are reused
                # unchanged; no second clustering operation is performed.
                rep_periods_data_red = copy(rep_periods_data_df)
                timeframe_data_red = copy(timeframe_data_df)

                @assert Set(unique(profiles_rep_periods_red.scenario)) ==
                        Set(selected_scenarios_ids)
                @assert Set(unique(rep_periods_mapping_red.scenario)) ==
                        Set(selected_scenarios_ids)
                @assert Set(unique(profiles_red.scenario)) ==
                        Set(selected_scenarios_ids)

                profiles_rep_periods_path = joinpath(
                    stochpath,
                    "profiles_rep_periods_reused.csv",
                )
                rep_periods_mapping_path = joinpath(
                    stochpath,
                    "rep_periods_mapping_reused.csv",
                )
                rep_periods_data_path = joinpath(
                    stochpath,
                    "rep_periods_data_reused.csv",
                )
                timeframe_data_path = joinpath(
                    stochpath,
                    "timeframe_data_reused.csv",
                )
                profiles_path_red = joinpath(
                    stochpath,
                    "profiles_reused.csv",
                )

                CSV.write(
                    profiles_rep_periods_path,
                    profiles_rep_periods_red;
                    writeheader=true,
                )
                CSV.write(
                    rep_periods_mapping_path,
                    rep_periods_mapping_red;
                    writeheader=true,
                )
                CSV.write(
                    rep_periods_data_path,
                    rep_periods_data_red;
                    writeheader=true,
                )
                CSV.write(
                    timeframe_data_path,
                ...
                DuckDB.query(
                    ...
                    SELECT *
                    FROM read_csv_auto('$(profiles_rep_periods_path)')
                    """,
                )
                ...
                DuckDB.query(
                    ...
                    SELECT *
                    FROM read_csv_auto('$(rep_periods_mapping_path)')
                    """,
                )
                ...
                DuckDB.query(
                    ...
                    SELECT *
                    FROM read_csv_auto('$(rep_periods_data_path)')
                    """,
                )
                ...
                DuckDB.query(
                    ...
                    SELECT *
                    FROM read_csv_auto('$(timeframe_data_path)')
                    """,
                )
                ...
                DuckDB.query(
                    ...
                    SELECT *
                    FROM read_csv_auto('$(profiles_path_red)')
                    """,
                )

                reduced_scenario_ids = sort(
                    collect(unique(TIO.get_table(connection, "stochastic_scenario").scenario)),
                )

                @assert reduced_scenario_ids == sort(collect(selected_scenarios_ids))
                @assert sort(collect(unique(
                    TIO.get_table(connection, "profiles_rep_periods").scenario,
                ))) == reduced_scenario_ids
                @assert sort(collect(unique(
                    TIO.get_table(connection, "rep_periods_mapping").scenario,
                ))) == reduced_scenario_ids
                @assert sort(collect(unique(
                    TIO.get_table(connection, "profiles").scenario,
                ))) == reduced_scenario_ids
                @assert nrow(TIO.get_table(connection, "rep_periods_data")) > 0
                @assert nrow(TIO.get_table(connection, "timeframe_data")) > 0
                ...
                if fix_benchmark
                    ...
                    if !isempty(outlier_df)
                        ...
                        # Keep original scenario IDs also for the enlarged reduced
                        # set that includes newly detected outlier scenarios.
                        df_stochastic_scenario_CC = DataFrame(
                            scenario=selected_scenarios_ids,
                            probability=probabilities,
                        )
                        ...
                        mkpath(joinpath(input_data_path_CC, "twice"))
                        ...
                        # ------------------------------------------------------------------
                        # Reuse the SAME full-set clustering for the enlarged reduced
                        # scenario set containing the detected outliers.
                        # Keep original scenario IDs; do not cluster again.
                        # ------------------------------------------------------------------
                        ...
                        # IMPORTANT: use stochpath_twice for the outlier branch.
                        profiles_rep_periods_path = joinpath(
                            stochpath_twice,
                            "profiles_rep_periods_reused.csv",
                        )
