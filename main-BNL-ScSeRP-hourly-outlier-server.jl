...
input_data_path = joinpath(
    @__DIR__,
    config["simulation"]["input_data_BNL"],
)

input_data_path_CC = joinpath(
    @__DIR__,
    config["simulation"]["input_data_BNL_CC"],
)
...
profiles_path = joinpath(@__DIR__, "profiles-wide-all-scenarios.csv")
...
function main()
    ...
    for solver in solvers
        ...
        baseline_output_folder = joinpath(@__DIR__, "BNL-output-data", base_name, "N$(number_of_scenarios)_draw$(draw)", string(solver))
    ...
    for row in eachrow(case_studies_info)
        ...
        for rp in representative_periods
            ...
            for solver in solvers
                ...
                output_folder = joinpath(@__DIR__, "BNL-output-data", "N$(number_of_scenarios)_draw$(draw)", case_name, string(solver))
                ...
                output_folder = joinpath(@__DIR__, "BNL-output-data", "N$(number_of_scenarios)_draw$(draw)", case_name, "scenario_selection", string(solver))
                mkpath(output_folder)
    ...
    output_folder = joinpath(@__DIR__, "BNL-output-data", "N$(number_of_scenarios)_draw$(draw)")
