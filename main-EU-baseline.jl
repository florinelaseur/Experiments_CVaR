...
function main()
    ...
    #     baseline_output_folder = joinpath(homedir(), "Nextcloud", "ExperimentData", "EU-output-data", base_name, "N$(number_of_scenarios)_draw$(draw)", string(solver))
    ...
    #             occursin("ens", lowercase(row.from_asset)) && occursin("h_demand", lowercase(row.to_asset)),
    #         var_flow_df
    #     )
    ...
    for row in eachrow(case_studies_info)
        ...
        for rp in representative_periods
            ...
            for solver in solvers
                ...
                output_folder = joinpath(homedir(), "Nextcloud", "ExperimentData", "EU-output-data", "N$(number_of_scenarios)_draw$(draw)", case_name, string(solver))
    ...
    output_folder = joinpath(homedir(), "Nextcloud", "ExperimentData", "EU-output-data", "N$(number_of_scenarios)_draw$(draw)")
    results_df |> CSV.write(joinpath(output_folder, "results_EU_N$(number_of_scenarios)_draw$(draw).csv"); writeheader=true)
