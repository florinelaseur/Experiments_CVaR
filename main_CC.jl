...
#for proxy
config_proxy = TOML.parsefile("config_proxy.toml")
input_data_path_proxy = config_proxy["simulation"]["input_data"]
use_ratio = config_proxy["clustering"]["use_ratio"]
heuristic_distance = config_proxy["clustering"]["heuristic_distance"]
fix_level_storage = config_proxy["simulation"]["fix_level_storage"]
representative_periods_proxy = config_proxy["simulation"]["representative_periods"]
solvers = [Symbol(el) for el in config_proxy["simulation"]["solvers"]]
lambda_proxy = config_proxy["simulation"]["risk_aversion_weight_lambda"]
alpha_proxy = config_proxy["simulation"]["risk_aversion_confidence_level"]
number_of_scenarios_proxy = config_proxy["simulation"]["number_of_scenarios"]
run_benchmark_proxy = config_proxy["simulation"]["run_benchmark"]
...
function main_proxy()
    ...
    case_studies_info =
        CSV.read(joinpath(@__DIR__, "case-studies-info-proxy.csv"), DataFrame)

    config = TOML.parsefile(joinpath(@__DIR__, "config_proxy.toml"))
    ...
    if run_benchmark
        ...
        time_to_read =
            @elapsed energy_problem_benchmark = TEM.EnergyProblem(connection_benchmark)
        ...
        for solver in solvers
            ...
            output_folder =
                joinpath(@__DIR__, "outputs_proxy", base_name, string(solver))
            ...
            mu_value_df =
                TIO.get_table(connection_benchmark, "var_value_at_risk_threshold_mu")
            ...
            flow_ens = filter(
                row -> row.from_asset == "ens" &&
                    row.to_asset == "e_demand",
                var_flow_df,
            )

            flow_smr_ccs = filter(
                row -> row.from_asset == "smr_ccs" &&
    ...
    for row in eachrow(case_studies_info)
        ...
        weight_fitting_kwargs =
            Dict(:learning_rate => learning_rate, :niters => niters)
        ...
        for rp in representative_periods
            ...
            for solver in solvers
                ...
                mu_value_df =
                    TIO.get_table(connection, "var_value_at_risk_threshold_mu")
    ...
    results_df |>
    CSV.write("outputs_proxy/results.csv"; writeheader=true)
...
# output_folder = joinpath(@__DIR__, "outputs-CC", base_name, string(solver))
# mkpath(output_folder)
...
# plot_cost_per_scenario(df_operational_cost_per_scenario, output_folder)
