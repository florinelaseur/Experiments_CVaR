...
config = TOML.parsefile("config_scn-gen.toml")
...
profiles_df = CSV.read(joinpath(@__DIR__, "base-input-data", "RIDM-case-study", "profiles-wide.csv"), DataFrame)
df_stochastic_scenario = CSV.read(joinpath(@__DIR__, "base-input-data", "RIDM-case-study", "stochastic-scenario.csv"), DataFrame)

tail_scenarios_path = joinpath(@__DIR__, "outputs_RA", "0_hourlyBenchmark", "Gurobi", "var_tail_excess_slack_xi.csv")
tail_scenarios_df = CSV.read(tail_scenarios_path, DataFrame)

tail_scenarios_ids = filter(row -> row.solution > 0.0, eachrow(tail_scenarios_df)).scenario
non_tail_scenarios_ids = filter(row -> row.solution == 0.0, eachrow(tail_scenarios_df)).scenario

# rng = MersenneTwister(1234)
# representative_non_tail_scenario = rand(rng, non_tail_scenarios_ids)

df_representative_non_tail_scenario = CSV.read(joinpath(@__DIR__, "outputs", "convex_conicalb_cross_rp_90", "Gurobi", "average-case-cost.csv"), DataFrame)
representative_non_tail_scenario = df_representative_non_tail_scenario.scenario
selected_scenarios_ids = vcat(tail_scenarios_ids, representative_non_tail_scenario)

profiles_df_reduced = filter(row -> row.scenario in selected_scenarios_ids, profiles_df)
CSV.write(joinpath(input_data_path, "profiles-wide.csv"), profiles_df_reduced; writeheader=true)

n_tail = length(tail_scenarios_ids)
tail_probability = (1.0 - alpha) / n_tail
df_stochastic_scenario_reduced = DataFrame(
    scenario=selected_scenarios_ids,
    probability=[fill(tail_probability, n_tail);
        alpha
    ],
)
...
function main()
    ...
    if run_benchmark
        ...
        for solver in solvers
            ...
            output_folder = joinpath(@__DIR__, "outputs_scn-gen-rp", base_name, string(solver))
    ...
    for row in eachrow(case_studies_info)
        ...
        for rp in representative_periods
            ...
            for solver in solvers
                ...
                output_folder = joinpath(@__DIR__, "outputs_scn-gen-rp", case_name, string(solver))
                mkpath(output_folder)
