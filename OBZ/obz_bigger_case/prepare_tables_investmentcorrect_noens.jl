...
connection = DBInterface.connect(DuckDB.DB, "obz6.db")
...
# for capacity --> battery = 77577.8503057667 / 4 = 19394.4625764417
# coal = 420000.0
# ocgt = 55000.0
# gas = 95000.0     
# nuclear = 950000.0
...
DuckDB.query(
    ...
    SELECT
        ...
        3 * a.capacity AS investment_limit,
        CASE
            WHEN LOWER(tay.name) LIKE '%battery%' THEN 19394.4625764417
            ELSE 0.0
        END AS investment_cost_storage_energy,
        CASE 
            WHEN LOWER(tay.name) LIKE '%battery%' THEN 2 * a.capacity_storage_energy
            ELSE 0.0
...
DuckDB.query(
    ...
    SELECT
        ...
        CASE
            WHEN LOWER(tay.name) LIKE '%battery%' THEN 0
            ELSE tay.initial_storage_units
        END as initial_storage_units
    FROM t_asset_yearly tay
    JOIN asset a
    ON a.asset = tay.name
    ORDER BY asset;
...
output_dir = joinpath(@__DIR__, "input_tables_noens_correctinv")
mkpath(output_dir)
