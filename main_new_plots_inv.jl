...
function main()
    ...
    if run_benchmark
        ...
        for solver in solvers
            ...
            output_folder = joinpath(@__DIR__, "outputs_inv", base_name, string(solver))
    ...
    for row in eachrow(case_studies_info)
        ...
        for rp in representative_periods
            ...
            for solver in solvers
                ...
                output_folder = joinpath(@__DIR__, "outputs_inv", case_name, string(solver))
                mkpath(output_folder)
                ...
                if run_benchmark
                    ...
                    output_folder = joinpath(@__DIR__, "outputs_inv", "fixed", case_name, string(solver))
                    mkpath(output_folder)
                    ...
                    benchmark_df = CSV.read(joinpath(@__DIR__, "outputs_inv", "0_HourlyBenchmark", "Gurobi", "benchmark_investment_decisions.csv"), DataFrame)
    ...
    output_folder = joinpath(@__DIR__, "outputs_inv", "fixed", "convex_conicalb_cross_rp_30", "Gurobi")
    ...
    results_df |> CSV.write("outputs_inv/results.csv"; writeheader=true)
