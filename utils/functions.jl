...
function compute_infeasibility_conflict!(model::JuMP.Model)
    status = JuMP.termination_status(model)
    if !(status in (JuMP.INFEASIBLE, JuMP.INFEASIBLE_OR_UNBOUNDED))
        @warn "compute_infeasibility_conflict! called but termination status is $status"
    ...
    if JuMP.get_attribute(model, MOI.ConflictStatus()) != MOI.CONFLICT_FOUND
        @warn "No infeasibility conflict (IIS) found by solver"
        return nothing, nothing
    end

    return JuMP.copy_conflict(model)
end

function _variable_bounds_in_conflict(model::JuMP.Model, var)
    in_conflict = false
    if JuMP.has_lower_bound(var)
        in_conflict =
            JuMP.get_attribute(JuMP.LowerBoundRef(var), MOI.ConstraintConflictStatus()) ==
            MOI.IN_CONFLICT
    end
    if !in_conflict && JuMP.has_upper_bound(var)
        in_conflict =
            JuMP.get_attribute(JuMP.UpperBoundRef(var), MOI.ConstraintConflictStatus()) ==
            MOI.IN_CONFLICT
    end
    if !in_conflict && JuMP.is_binary(var)
        in_conflict =
            JuMP.get_attribute(JuMP.IntegralityRef(var), MOI.ConstraintConflictStatus()) ==
            MOI.IN_CONFLICT
    elseif !in_conflict && JuMP.is_integer(var)
        in_conflict =
            JuMP.get_attribute(JuMP.IntegralityRef(var), MOI.ConstraintConflictStatus()) ==
            MOI.IN_CONFLICT
    end
...
function print_infeasibility_conflict!(model::JuMP.Model; max_lines=50)
    status = JuMP.termination_status(model)
    primal = JuMP.primal_status(model)

    iis_model, reference_map = compute_infeasibility_conflict!(model)
    if iis_model === nothing
        return nothing, nothing
    end
    ...
    for var in JuMP.all_variables(model)
        _variable_bounds_in_conflict(model, var) && (n_bounds += 1)
    end

    @warn "Infeasibility conflict (IIS) — $n_con constraints, $n_bounds variable bounds in conflict (status=$status, primal=$primal)"
    ...
    for (F, S) in JuMP.list_of_constraint_types(model)
        for con in JuMP.all_constraints(model, F, S)
            if JuMP.get_attribute(con, MOI.ConstraintConflictStatus()) == MOI.IN_CONFLICT
                ...
                con_name = JuMP.name(con)
                con_str = JuMP.constraint_string(MIME("text/plain"), con)
                @info "[constraint] $(isempty(con_name) ? con : con_name)" con_str
                lines += 1
            end
        end
        lines >= max_lines && break
    ...
    for var in JuMP.all_variables(model)
        if _variable_bounds_in_conflict(model, var)
            lines >= max_lines && break
            lb = JuMP.has_lower_bound(var) ? JuMP.lower_bound(var) : -Inf
            ub = JuMP.has_upper_bound(var) ? JuMP.upper_bound(var) : Inf
            fixed = JuMP.is_fixed(var) ? JuMP.fix_value(var) : nothing
            @info "[bound] $(JuMP.name(var))" lb=lb ub=ub fixed=fixed
            lines += 1
        end
    end
    ...
    return iis_model, reference_map
end
