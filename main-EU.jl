...
function apply_location_ratio_to_profiles_wide!(profiles_df::DataFrame)
    demand_columns = [col for col in names(profiles_df) if occursin(r"_E_Demand$", string(col))]

    for demand_col in demand_columns
        location_prefix = first(split(string(demand_col), "_"))
        for col in names(profiles_df)
            col_name = string(col)
            if col_name == demand_col
                continue
            end
            if startswith(col_name, location_prefix * "_")
                profiles_df[!, col] = profiles_df[!, col] ./ profiles_df[!, demand_col]
            end
        end
    end
...
function apply_location_ratio_to_long_profiles!(connection, table_name::String; use_rep_period::Bool=false)
    join_clause = use_rep_period ? "AND d.rep_period = x.rep_period" : ""
    ...
    DuckDB.query(
        ...
        SET value =
            CASE
                WHEN x.profile_name = CONCAT(split_part(x.profile_name, '_', 1), '_E_Demand')
                    THEN x.value
        ...
        WHERE d.timestep = x.timestep
          ...
          $join_clause
          AND d.profile_name = CONCAT(split_part(x.profile_name, '_', 1), '_E_Demand');
        """,
    )
end
...
use_ratio = Bool(config["clustering"]["use_ratio"])
...
profiles_path = joinpath(data_root, "profiles-wide-all-scenarios-EU.csv")
all_profiles_df = CSV.read(profiles_path, DataFrame)
profiles_df = get_scenario_set(all_profiles_df, number_of_scenarios)
if use_ratio
    apply_location_ratio_to_profiles_wide!(profiles_df)
end
...
function main()
    ...
    for row in eachrow(case_studies_info)
        ...
        for rp in representative_periods
            ...
            # The wide-format profiles already contain the location-wise ratio values.
            # transform the profiles data from wide to long
            TC.transform_wide_to_long!(
                connection_benchmark,
                "profiles_wide",
            ...
            if use_ratio
                apply_location_ratio_to_long_profiles!(connection_benchmark, "profiles")
            end
            ...
            for solver in solvers
                ...
                if stochastic_method == :per_scenario
                    ...
                    if use_ratio
                        apply_location_ratio_to_long_profiles!(connection, "profiles_rep_periods"; use_rep_period=true)
                    end
                ...
                if use_ratio
                    apply_location_ratio_to_long_profiles!(connection, "profiles")
                end
    ...
    results_df |> CSV.write("outputs/results_ScSeRP_N$(number_of_scenarios)_seed$(seed).csv"; writeheader=true)
